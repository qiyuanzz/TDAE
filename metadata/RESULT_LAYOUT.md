...
wsi_TRANSMIL_keep_0.4_gc_32_lr_2e-4
wsi_CLAM_keep_1_gc_32_lr_2e-4
```

Use this level for model family, patch keep ratio, effective survival batch/gradient-count setting, and learning rate. Put `fold_0` through `fold_4` under the setting/seed directory, not above it.
...
- `split.csv`: exact fold split copied from `metadata/splits`.
...
- `scripts/step_04_train.py` defaults to `--output_layout seed` and saves patch-budget settings as `wsi_<AGGREGATOR>_keep_<ratio>_gc_<batch>_lr_<lr>`.
- `scripts/step_05_evaluate.py` evaluates checkpoints produced by `step_04_train.py`.
